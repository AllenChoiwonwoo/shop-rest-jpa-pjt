...
public class ProductService {
	...
	public ResultDTO getProdList(ProdNumbAndTokenDTO prodnumbntoken) {
		...
		logger.info("@@getProdList, token = "+prodnumbntoken.getAccesstoken());
		UserVO uservo = new UserVO();
		boolean isUser = false;
		try {
			uservo = userRepo.findUserVOByToken(prodnumbntoken.getAccesstoken());
			if(uservo == null) {
				logger.info("getProdList , 틀린 token 입니다.");
			}else {
				isUser = true;
			}
			
		} catch (Exception e) {
			logger.info("getProdList , 토큰이 안왔습니다.");
			uservo = new UserVO();
		}
//		logger.info("getProdList , "+uservo.toString());
...
//		System.out.println("prodListpaged.size = "+ prodListPaged.size());
		...
		if(isUser ) { // 로그인 중이라면
			logger.info("이 안으로 들어가는겨? token 이 없어도?");
			uservo = userRepo.findUserVOByToken(prodnumbntoken.getAccesstoken());
			logger.info("getProdList , "+uservo.toString());
			...
			logger.info("getProdList , 3");
			pagedProdDTO.setUserId(uservo.getId());
			logger.info("getProdList , 4");

			return ShopRestJpaServerApplication.returnSuccess(pagedProdDTO);
		}else {
			logger.info("getProdList , 1");

			prodDTOList = new ArrayList<ProductDTO>();
			for (ProductVO productVO : prodListPaged) {
				prodDTOList.add(new ProductDTO().setValue(productVO));
			}
			logger.info("getProdList , 2");

			// prodDTOList 의 모든 isInBasket 값은 false
			pagedProdDTO.setListProdDTO(prodDTOList);
			logger.info("getProdList , 3");
			pagedProdDTO.setUserId(0);
			logger.info("getProdList , 4");
			return ShopRestJpaServerApplication.returnSuccess(pagedProdDTO);
		}
...
//		prodnumbntoken.setProdid(3);
		...
		if(prodnumbntoken.getAccesstoken() != null) {//로그인 중일 경우
			...
			System.out.println("accesstoken = "+prodnumbntoken.getAccesstoken());
